...
/// @title Rookie Edge
/// @notice Retail-first dynamic fee controller with toxicity and directional protection.
/// @dev Built to improve edge by trading a bit more arb risk for much higher retail capture.
contract Strategy is AMMStrategyBase {
    // --- decay / update constants ---
    uint256 private constant ELAPSED_CAP = 8;
    uint256 private constant SIGNAL_THRESHOLD = WAD / 700; // ~14 bps reserve ratio
    uint256 private constant DIR_DECAY = 840000000000000000; // 0.84
    uint256 private constant ACT_DECAY = 750000000000000000; // 0.75
    uint256 private constant SIZE_DECAY = 720000000000000000; // 0.72
    uint256 private constant TOX_DECAY = 820000000000000000; // 0.82
    uint256 private constant SIGMA_DECAY = 680000000000000000; // 0.68
    uint256 private constant LAMBDA_DECAY = 990000000000000000; // 0.99
    uint256 private constant SIZE_BLEND_DECAY = 700000000000000000; // 0.70
    uint256 private constant TOX_BLEND_DECAY = 200000000000000000; // 0.20
    uint256 private constant ACT_BLEND_DECAY = 985000000000000000; // 0.985
    uint256 private constant PHAT_ALPHA = 240000000000000000; // 0.24
    uint256 private constant PHAT_SHOCK_GATE = 40000000000000000; // 4%
    ...
    uint256 private constant LAMBDA_CAP = 5 * WAD; // max 5 trades/step estimate
    uint256 private constant STEP_COUNT_CAP = 64;

    // --- fee model constants ---
    uint256 private constant BASE_FEE = 2 * BPS;
    uint256 private constant SIGMA_COEF = 160000000000000000; // 0.16
    uint256 private constant LAMBDA_COEF = 10 * BPS;
    uint256 private constant FLOW_SIZE_COEF = 6200 * BPS;
    uint256 private constant TOX_COEF = 280 * BPS;
    uint256 private constant TOX_QUAD_COEF = 10800 * BPS;
    uint256 private constant ACT_COEF = 50000 * BPS;
    uint256 private constant DIR_COEF = 24 * BPS;
    uint256 private constant DIR_TOX_COEF = 120 * BPS;
    uint256 private constant STALE_DIR_COEF = 6500 * BPS;
    uint256 private constant TAIL_KNEE = 420 * BPS;
    uint256 private constant TAIL_SLOPE = 880000000000000000; // 0.88

    // Competitive re-entry: if we were idle for many steps, tighten to win flow back.
    uint256 private constant IDLE_DISCOUNT_PER_STEP = 1 * BPS;
    uint256 private constant IDLE_DISCOUNT_CAP = 6 * BPS;
        ...
        returns (uint256, uint256)
    {
        ...
        slots[8] = 1800000000000000; // 0.18% reserve-size ratio guess
        slots[9] = 0;
        slots[10] = 0;
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 elapsedRaw = 0;
        if (isNewStep) {
            elapsedRaw = trade.timestamp - lastTs;
        ...
        // Recover implied fair level from traded side and fee in use.
        ...
        if (tradeRatio > SIGNAL_THRESHOLD) {
            ...
            // isBuy=true => AMM buys X => local sell pressure.
            if (trade.isBuy) {
        ...
        if (isNewStep && elapsedRaw > 1) {
            uint256 idleDiscount = (elapsedRaw - 1) * IDLE_DISCOUNT_PER_STEP;
            if (idleDiscount > IDLE_DISCOUNT_CAP) idleDiscount = IDLE_DISCOUNT_CAP;
            fBase = fBase > idleDiscount ? fBase - idleDiscount : 0;
        }
        ...
        // Additional stale-direction protection around pHat.
